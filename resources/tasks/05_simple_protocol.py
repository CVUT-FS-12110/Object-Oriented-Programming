import struct
import io

"""
    vvvv      YOUR SOLUTION      vvvv
"""


def parse_data(input_binary_data):  # dont change the signature of the function

    # use this function to run your solution

    return "decoded message"  # here return decoded message in correct format


"""
    ^^^^      YOUR SOLUTION      ^^^^
#################################################################
    vvvv TESTS FOR YOUR SOLUTION vvvv
"""

assert parse_data(b'\x01\x00\x0B\x00\x02\x01\x00\x05\x48\x65\x6C\x6C\x6F\x01\x00\x06\x57\x6F\x72\x6C\x64\x21') == "{FrameHeader[1,11,0,2]:FramePayload[<SubFrameHeader[1,5]:SubFrameData[Hello]>,<SubFrameHeader[1,6]:SubFrameData[World!]>]}->Hello World!"
assert parse_data(b'\x01\x00\x00\x00\x00') == '{FrameHeader[1,0,0,0]:FramePayload[]}->'
assert parse_data(b'\x01\x00\x0B\x00\x02\x01\x00\x05\x48\x65\x6C\x6C\x6F\x01\x00\x06\x57\x6F\x72\x6C\x64\x21\x01\x00\x0B\x00\x02\x01\x00\x05\x48\x65\x6C\x6C\x6F\x01\x00\x06\x57\x6F\x72\x6C\x64\x21') == "{FrameHeader[1,11,0,2]:FramePayload[<SubFrameHeader[1,5]:SubFrameData[Hello]>,<SubFrameHeader[1,6]:SubFrameData[World!]>]}->Hello World!\n{FrameHeader[1,11,0,2]:FramePayload[<SubFrameHeader[1,5]:SubFrameData[Hello]>,<SubFrameHeader[1,6]:SubFrameData[World!]>]}->Hello World!"
assert parse_data(b'') == "{FrameHeader[1,50,0,4]:FramePayload[<SubFrameHeader[1,13]:SubFrameData[Hustodemonsky]>,<SubFrameHeader[1,11]:SubFrameData[krutoprisna]>,<SubFrameHeader[1,17]:SubFrameData[monstrne-vtipozni]>,<SubFrameHeader[1,9]:SubFrameData[aliofobie]>]}->Hustodemonsky krutoprisna monstrne-vtipozni aliofobie"
